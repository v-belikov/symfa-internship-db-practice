...
* **MySQL**
  * Install [MySQL](https://dev.mysql.com/downloads/mysql/5.5.html) or use docker (see below). **That's important:** The tests checked on Mysql 5.7 and Mysql 8.0 only.
...
* **MongoDB**
  * Download and install [MongoDB community serrver](https://www.mongodb.com/try/download/community) or use docker (see below)
...
* **Docker**
  ...
  * Restore MySql db using `adminer` (`localhost:8088`) or see `docker-compose.yml`.
  * Restore MongoDB db using `mongo-express` (`localhost:8089`) or see `docker-compose.yml`.
